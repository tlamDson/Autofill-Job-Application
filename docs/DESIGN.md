...
2. ~~Resume upload chưa nối vào runtime~~ — đã nối vào generic path (`feature/resume-upload`), độc lập với các hàm `upload*Resume` per-ATS (vẫn dead code, xem mục 6). `options/sections/documents.js` lưu 1 resume "hiện tại" vào IndexedDB (`resumeStore.js`, nguồn dữ liệu chính cho options page) **và** mirror bytes sang `chrome.storage.local` base64 (`resumeTransport.js`) — content script không đọc được IndexedDB của options page vì khác origin (trang options chạy ở `chrome-extension://`, content script thấy origin của trang web đang mở), còn `chrome.storage.local` là API extension thật, không bị chia theo origin trang. `content.js` đọc mirror này thành `File`, gửi kèm trong `window.postMessage` payload (structured clone giữ nguyên `File`) sang `injected.js`, gắn vào bản clone runtime-only của profile (`profile.__resumeFile`, không bao giờ lưu lại storage). `matcher.js` có rule `resume` (chỉ khớp `input[type=file]`), `generic.js` gọi `attachFileToInput` + `resolveResumeFileName` (đã có sẵn từ trước, giờ mới thực sự được gọi).
